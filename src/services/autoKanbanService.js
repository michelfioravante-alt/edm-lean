...
export const autoKanbanService = {
    async fetchAll() {
        if (isLocalMode()) return localApi.autoKanban.fetchAll();
    ...
    async create({ tipo, descricao, diasIntervalo, maquinaNome }) {
        if (isLocalMode()) return localApi.autoKanban.create({ tipo, descricao, diasIntervalo, maquinaNome });
        const empresaId = getEmpresaId();
        const { data, error } = await supabase
            .from('kanbans_automaticos')
            .insert({
    ...
    async remove(id) {
        if (isLocalMode()) return localApi.autoKanban.remove(id);
        const empresaId = getEmpresaId();
